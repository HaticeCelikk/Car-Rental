...
public class RentScreen extends JFrame {
	...
	public RentScreen(int start) {
		...
		btnNewButton_1.addMouseListener(new MouseAdapter() {
			@Override
			public void mouseClicked(MouseEvent e) { // when customer wants go back for one page
		...
		JRadioButton rdbtnNewRadioButton = new JRadioButton("Click to change budget");// if customer wants to change
																						// budget
		...
		rdbtnNewRadioButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				int newBudget = Integer.valueOf(str);
				Login.customer.setBudget(newBudget);
				RentScreen r = new RentScreen(0);
				r.setVisible(true);
				setVisible(false);
		...
		int pages = (Login.company.getCarsForRent().size() / 6);
		if ((Login.company.getCarsForRent().size() / 6) % 6 != 0) {
			pages++;
		}
		...
		comboBox.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				RentScreen r = new RentScreen(page);
				r.setVisible(true);
				setVisible(false);
			}
		...
		JLabel lblNewLabel_9 = new JLabel("RENT");
	...
	public void writingRent(int startCar) { // funtcion for writing to one page at most 6 car
		int x = 0;
		int y = 35;
		int size = 6;
		if (startCar + 6 > Login.company.getCarsForRent().size()) {
			size = Login.company.getCarsForRent().size() - startCar;
		}
		for (int i = startCar; i < startCar + size; i++) {
			...
			JLabel lblNewLabel_1 = new JLabel(
					"Color            :            " + Login.company.getCarsForRent().get(i).getColor());
			lblNewLabel_1.setForeground(Color.WHITE);
			lblNewLabel_1.setBounds(43, 170, 247, 13);
			panel.add(lblNewLabel_1);

			JLabel lblNewLabel_2 = new JLabel(
					"Engine         :            " + Login.company.getCarsForRent().get(i).getEngineSize() + "    /   "
			...
			lblNewLabel_3.setBounds(20, 220, 247, 13);
			lblNewLabel_3.setHorizontalAlignment(SwingConstants.CENTER);
			panel.add(lblNewLabel_3);
			// 59 85 200 200

			ImageIcon image_2 = new ImageIcon(Login.company.getCarsForRent().get(i).getImage());
			...
			JLabel lblNewLabel_1_1 = new JLabel(
					Login.company.getCarsForRent().get(i).getPricePerHour() + " TRY Per Day");
				...
				public void mouseClicked(MouseEvent e) {
					for (int i = 0; i < Login.company.getCarsForRent().size(); i++) {
						...
						if (panel.getName().equalsIgnoreCase(tempCar)) {
							RentContract page = new RentContract(Login.company.getCarsForRent().get(i));
